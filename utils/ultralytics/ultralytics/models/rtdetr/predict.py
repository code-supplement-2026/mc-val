...
class RTDETRPredictor(BasePredictor):
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        results = []
        self.full_probs = []      # --- NEWNEW from Adam 2
        for bbox, score, orig_img, img_path in zip(bboxes, scores, orig_imgs, self.batch[0]):  # (300, 4)
            ...
            # 🎯 2. FILTER the full probability vectors using the SAME index `idx`
            filtered_probs = score[idx]

            # 3. APPEND the filtered vectors for this image to your list
            self.full_probs.append(filtered_probs)
#            self.full_probs = filtered_probs
            # --- END OF MODIFICATION ---
