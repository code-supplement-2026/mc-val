...
def non_max_suppression(
    ...
    return_probs: bool = False,         # --- NEWNEW from Adam
):
    """
    ...
    probs_output = [torch.zeros((0, nc), device=prediction.device)] * bs if return_probs else None          # --- NEWNEW from Adam
    ...
    for xi, (x, xk) in enumerate(zip(prediction, xinds)):  # image index, (preds, preds indices)
        ...
        # This is the perfect place to get the full probability distribution
        # --- NEWNEW from Adam
        if return_probs:
            all_probs = cls.clone()
#            all_probs = cls.softmax(1)
...
#    return (output, keepi) if return_idxs else output
    ...
    # --- NEWNEW from Adam
    if return_idxs and return_probs:
        return output, keepi, probs_output
    elif return_idxs:
        return output, keepi
    elif return_probs:
        return output, probs_output
    else:
        return output
